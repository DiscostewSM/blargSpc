...
void DspPrepareStateAfterReload() {
	...
	DspWriteByte(DSP_MEM[DSP_PMOD], DSP_PMOD);
	DspWriteByte(DSP_MEM[DSP_FLAG], DSP_FLAG);
	DspWriteByte(DSP_MEM[DSP_NON], DSP_NON);
	...
	for(i = 0; i < 8; i++)
		DspSetFIRCoefficient(i);

	


	//printf("[0x4C] - %d, [0x5C] - %d\n", DSP_MEM[0x4C], DSP_MEM[0x5C]);
}
